...
int main() {
    ...
    InitHashTable();
    FILE* file = fopen("shakespeare_part.txt", "r");
    fscanf(file, "%s", word1);
    ...
    while (fscanf(file, "%s", word2) == 1){  
        ...
        snprintf(bigram, sizeof(bigram), "%s %s", word1, word2); 
        int bucketIndex = hash(bigram); 
        BigramNode* node = hashTable[bucketIndex];
        node = rScanList(node, bigram);

        strcpy(word1, word2); // 연이어서 bigram 설정
    }
    fclose(file);

    // printf("Now insertion sorting...\n");
    InsertionSort();
    printBigrams(hashTable[0]);
	// printBigramsToFile(hashTable[0]);
